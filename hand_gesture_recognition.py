...
def gesture():
    ...
    while 1:
        ...
        l_b = np.array([0, 23, 42], dtype=np.uint8)
        u_b = np.array([255, 199, 255], dtype=np.uint8)
        cvt = cv2.inRange(cvt, l_b, u_b)
        cvt = cv2.erode(cvt, kernel, iterations=4)
        ...
        if (len(areas) != 0):
            ...
            if (areas[maxi] > 10000):
                ...
                for i in range(defects.shape[0]):
                    ...
                    a = math.sqrt(pow((end[0] - st[0]) ,2) + pow((end[1] - st[1]) ,2))
                    b = math.sqrt(pow((f[0] - st[0]) ,2) + pow((f[1] - st[1]) , 2))
                    c = math.sqrt(pow((end[0] - f[0]) ,2 )+ pow((end[1] - f[1]) , 2))
                ...
                if (num == 1):
                    ...
                    gesturecnt[num] += 1
                    if (gesturecnt[num] > 40):
                        os.chdir(r"C:\Users\91945\Desktop")
